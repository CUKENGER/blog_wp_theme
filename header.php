...
<head>
  ...
  <link rel="icon" href="<?php echo esc_url(
    ...
    type="image/x-icon">
  <link rel="icon" href="<?php echo esc_url(
      get_template_directory_uri() . "/assets/favicon.svg",
  ); ?>"
    type="image/svg+xml">
  ...
  <script type="text/javascript">
      ...
      })(window, document,'script','https://mc.yandex.ru/metrika/tag.js?id=104860628', 'ym');

      ym(104860628, 'init', {ssr:true, webvisor:true, clickmap:true, ecommerce:"dataLayer", accurateTrackBounce:true, trackLinks:true});
  </script>
  <noscript><div><img src="https://mc.yandex.ru/watch/104860628" style="position:absolute; left:-9999px;" alt="" /></div></noscript>

  <?php
  $site_name = get_bloginfo("name"); // Название сайта
  $site_description = get_theme_mod(
      "tgx_site_description",
      get_bloginfo("description"),
  ); // Глобальное описание из Customizer
  $site_keywords = get_theme_mod(
      "tgx_site_keywords",
      "монетизация, telegram, заработок, блог",
  ...
  if (is_front_page() || is_home()) {
      ...
      $description =
          $site_description ?:
          "Блог о монетизации Telegram, заработке и развитии каналов";
      $keywords = $site_keywords;
  } elseif (is_single()) {
      $custom_title = get_post_meta(get_the_ID(), "_tgx_seo_title", true);
      $custom_description = get_post_meta(
          get_the_ID(),
          "_tgx_seo_description",
          true,
      );
      $custom_keywords = get_post_meta(get_the_ID(), "_tgx_seo_keywords", true);
      $title = $custom_title ?: get_the_title() . " | " . $site_name;
      $description =
          $custom_description ?: wp_trim_words(get_the_excerpt(), 30, "...");
      $keywords =
          $custom_keywords ?:
          implode(", ", wp_list_pluck(get_the_category(), "name")) ?:
          "telegram, монетизация";
  } elseif (is_category()) {
      $category = get_queried_object();
      $category_description = get_term_meta(
          $category->term_id,
          "_tgx_category_description",
          true,
      );
      $category_keywords = get_term_meta(
          $category->term_id,
          "_tgx_category_keywords",
          true,
      );
    ...
    <div class="header-content">
      <a href="<?php echo esc_url(home_url("/")); ?>" class="header-logo">
        <img src="<?php echo esc_url(
            get_theme_mod(
                "tgx_logo",
                get_template_directory_uri() . "/assets/logo.svg",
            ),
        ); ?>"
          alt="Logo">
      </a>
      <div class="header-right">
        <div class="header-search-container">
          <div class="header-search-wrapper">
            <?php
            $search_icon = get_theme_mod(
                "tgx_search_icon",
                get_template_directory_uri() . "/assets/search-input.svg",
            );
            $search_open_icon = get_theme_mod(
                "tgx_search_icon",
                get_template_directory_uri() . "/assets/search.svg",
            );
            $search_icon_path = str_replace(
                get_site_url(),
                ABSPATH,
                $search_icon,
            );
            $search_open_icon_path = str_replace(
                get_site_url(),
                ABSPATH,
                $search_open_icon,
            );
        ...
        <div class="search-icon-container">
          <?php if (file_exists($search_open_icon_path)) {
        ...
        <a href="<?php echo esc_url(
            get_theme_mod(
                "tgx_telegram_link",
                "https://t.me/+OZZKJb_5At42NzJi",
            ),
        ); ?>"
          class="tg-icon-container" target="_blank" rel="noopener noreferrer">
          <?php
          $telegram_icon = get_theme_mod(
              "tgx_telegram_icon",
              get_template_directory_uri() . "/assets/tg-button.svg",
          );
          $telegram_icon_path = str_replace(
              get_site_url(),
              ABSPATH,
              $telegram_icon,
          );
    ...
    <div class="search-overlay-content">
      <form class="search-overlay-form">
        <div class="search-overlay-input-wrapper">
          <?php if (file_exists($search_icon_path)) {
              $svg_content = file_get_contents($search_icon_path);
              echo $svg_content;
          } else {
              error_log("Search icon not found: " . $search_icon_path);
